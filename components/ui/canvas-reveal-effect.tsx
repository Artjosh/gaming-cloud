...
const ShaderMaterial = ({
  ...
  let lastFrameTime = 0
  const [mousePosition, setMousePosition] = useState<[number, number]>([-1000, -1000])

  // Use React Three Fiber's useThree and useFrame for mouse tracking
  ...
  useFrame(({ clock, mouse }) => {
    ...
    // Convert normalized mouse coordinates to pixel coordinates
    const x = ((mouse.x + 1) / 2) * size.width
    const y = ((1 - mouse.y) / 2) * size.height

    // Update mouse position in shader
    if (!material.uniforms.u_mouse) {
      material.uniforms.u_mouse = { value: new THREE.Vector2(x, y) }
    } else {
